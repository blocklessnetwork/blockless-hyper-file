...
impl HttpRange {
    ...
    fn parse_single_range(range: &[u8], file_size: u64) -> Result<Option<HttpRange>> {
        ...
        let range_start = split_range.next()
            .ok_or(ParseError::InvalidRange)?;
        let range_end = split_range.next()
            .ok_or(ParseError::InvalidRange)?;
        if range_start.is_empty() {
            if range_end.is_empty() || range_end[0] == b'-' {
                return Err(ParseError::InvalidRange);
            }
            let mut length = range_end.to_u64()
                .ok_or(ParseError::InvalidRange)?;
            if length == 0 {
                return Ok(None);
            }
...
mod test {
    ...
    fn test_parse() {
        ...
        test_range!("bytes=-5", 10, vec![HttpRange{start: 5, length: 5}]);
        test_range!("bytes=0-5", 10, vec![HttpRange{start: 0, length: 6}]);
        test_range!("bytes=0-100", 10, vec![HttpRange{start: 0, length: 10}]);
        test_range!("bytes=0-", 10, vec![HttpRange{start: 0, length: 10}]);
        test_range!("bytes=   0- ", 10, vec![HttpRange{start: 0, length: 10}]);
        test_range!("bytes=   0-2 , 5-10", 10, vec![HttpRange{start: 0, length: 3}, HttpRange{start: 5, length: 5}]);
        test_range!("bytes=500-600,601-999", 1000, vec![HttpRange{start: 500, length: 101}, HttpRange{start: 601, length: 399}]);
    }
}